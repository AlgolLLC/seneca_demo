...
function $bucketAuto (collection, expr) {
  ...
  let approxBucketSize = Math.max(1, Math.round(collection.length / bucketCount));
...
function $group (collection, expr) {
  ...
  let partitions = groupBy(collection, (obj) => computeValue(obj, objectId, objectId));
...
function $redact (collection, expr) {
  return map(collection, (obj) => redactObj(cloneDeep(obj), expr))
}
